...
void loop() {
  ...
  long humidity = getHumidityPercentage(event);
  double temperature = getTemperatureCelsius(event);
  Serial.println("==================");
...
long getHumidityPercentage(sensors_event_t event) {
   int humiditySensorOutput;

  dht.humidity().getEvent(&event);
   if(!isnan(dht.humidity().getEvent(&event))){
       humiditySensorOutput = event.relative_humidity;
   }
...
double getTemperatureCelsius(sensors_event_t event) {
    int tempCelsius;

    dht.temperature().getEvent(&event);
    if(!isnan(dht.temperature().getEvent(&event))){
        tempCelsius = event.temperature;
    }
